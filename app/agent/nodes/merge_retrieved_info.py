...
async def merge_retrieved_info(state: DataAgentState, runtime: Runtime[DataAgentContext]):
    ...
    将召回的列信息、字段取值信息和指标信息合并为统一的表信息和指标信息结构。

    :param state: DataAgentState，包含retrieved_column_infos、retrieved_value_infos、retrieved_metric_infos等状态信息
    :param runtime: Runtime[DataAgentContext]，包含meta_mysql_repository等上下文
    :return: dict，包含table_infos和metric_infos键，值为合并后的表信息列表和指标信息列表
    """
    writer = runtime.stream_writer
    writer({"type": "progress", "step": "合并召回信息", "status": "running"})

    # 已召回信息
    retrieved_columns = state["retrieved_column_infos"]
    retrieved_values = state["retrieved_value_infos"] 
    retrieved_metrics = state["retrieved_metric_infos"]
